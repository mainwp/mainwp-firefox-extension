$(document).ready(() => {
    ...
    $('.save_settings').click(async () => {
        ...
        let url = encodeURIComponent(`${dashboard_url}/wp-json/mainwp/v1/sites/all-sites-count?consumer_key=${consumer_key}&consumer_secret=${consumer_secret}`);
        let result = await fetch(`https://cors.mainwp.com/proxy/${url}`, requestOptions)
        ...
        browser.storage.sync.set({ "settings": JSON.stringify(settings) }, function() {
            browser.runtime.sendMessage({ type: "updateCron" });
    ...
    const ShowUpdatesPage = (settings) => {
        ...
        browser.storage.sync.get(['last_updated'], (result) => {
